...
#include "process_repeat_usertype_dynarg_xor.h"
...
void process_repeat_usertype_dynarg_xor_t::_read() {
    ...
    for (int i = 0; i < l_blocks; i++) {
        ...
        m__raw_blocks->push_back(std::move(kaitai::kstream::process_xor_one(m__raw__raw_blocks->at(m__raw__raw_blocks->size() - 1), 155 ^ (i << 4 | _io()->pos()))));
...
void process_repeat_usertype_dynarg_xor_t::block_t::_read() {
    m_a = m__io->read_u4le();
}
...
void process_repeat_usertype_dynarg_xor_t::blocks_b_wrapper_t::_read() {
...
uint8_t process_repeat_usertype_dynarg_xor_t::blocks_b_wrapper_t::blocks_1_b() {
    if (f_blocks_1_b)
        return m_blocks_1_b;
    f_blocks_1_b = true;
