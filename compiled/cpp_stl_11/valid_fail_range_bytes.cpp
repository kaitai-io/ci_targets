...
void valid_fail_range_bytes_t::_read() {
    ...
    if (!(m_foo >= std::string("\x50", 1))) {
        throw kaitai::validation_less_than_error<std::string>(std::string("\x50", 1), m_foo, m__io, std::string("/seq/0"));
    }
    if (!(m_foo <= std::string("\x50\x31", 2))) {
        throw kaitai::validation_greater_than_error<std::string>(std::string("\x50\x31", 2), m_foo, m__io, std::string("/seq/0"));
    }
}
