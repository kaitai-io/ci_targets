...
void switch_repeat_expr_t::_read() {
    m_codes = std::unique_ptr<std::vector<uint8_t>>(new std::vector<uint8_t>());
    const int l_codes = 3;
    for (int i = 0; i < l_codes; i++) {
        m_codes->push_back(std::move(m__io->read_u1()));
    }
    ...
    for (int i = 0; i < l_body; i++) {
        switch (codes()->at(i)) {
        case 1: {
            m__raw_body->push_back(std::move(m__io->read_bytes(4)));
