...
type_ternary_opaque_t::type_ternary_opaque_t(kaitai::kstream* p__io, std::unique_ptr<kaitai::kstruct> p__parent, type_ternary_opaque_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = this;
...
void type_ternary_opaque_t::_read() {
    ...
    if (!(is_hack())) {
        ...
        m_dif_wo_hack = std::unique_ptr<term_strz_t>(new term_strz_t(m__io__raw_dif_wo_hack));
    }
    ...
    if (is_hack()) {
        ...
        m_dif_with_hack = std::unique_ptr<term_strz_t>(new term_strz_t(m__io__raw_dif_with_hack));
    }
}
