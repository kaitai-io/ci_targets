...
public class StrPadTermZeroSize extends KaitaiStruct.ReadWrite {
    ...
    public StrPadTermZeroSize(KaitaiStream _io, KaitaiStruct.ReadWrite _parent) {
        this(_io, _parent, null);
    }

    public StrPadTermZeroSize(KaitaiStream _io, KaitaiStruct.ReadWrite _parent, StrPadTermZeroSize _root) {
        super(_io);
    ...
    public void _write_Seq() {
        _assertNotDirty();
        this._io.writeBytesLimit((this.strPad).getBytes(Charset.forName("UTF-8")), 0, (byte) 64, (byte) 64);
        this._io.writeBytesLimit((this.strTerm).getBytes(Charset.forName("UTF-8")), 0, (byte) 64, (byte) 0);
        this._io.writeBytesLimit((this.strTermAndPad).getBytes(Charset.forName("UTF-8")), 0, (byte) 64, (byte) 43);
        this._io.writeBytesLimit((this.strTermInclude).getBytes(Charset.forName("UTF-8")), 0, (byte) 0, (byte) 0);
    }

    public void _check() {
        if ((this.strPad).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_pad", 0, (this.strPad).getBytes(Charset.forName("UTF-8")).length);
        ...
        if ((this.strTerm).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_term", 0, (this.strTerm).getBytes(Charset.forName("UTF-8")).length);
        ...
        if ((this.strTermAndPad).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_term_and_pad", 0, (this.strTermAndPad).getBytes(Charset.forName("UTF-8")).length);
    ...
    public void set_root(StrPadTermZeroSize _v) { _dirty = true; _root = _v; }
