...
public class StrPadTermZeroSize extends KaitaiStruct.ReadWrite {
    ...
    public void _check() {
        if ((this.strPad).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_pad", (this.strPad).getBytes(Charset.forName("UTF-8")).length, 0);
        ...
        if ((this.strTerm).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_term", (this.strTerm).getBytes(Charset.forName("UTF-8")).length, 0);
        ...
        if ((this.strTermAndPad).getBytes(Charset.forName("UTF-8")).length > 0)
            throw new ConsistencyError("str_term_and_pad", (this.strTermAndPad).getBytes(Charset.forName("UTF-8")).length, 0);
