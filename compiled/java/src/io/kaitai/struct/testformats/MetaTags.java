// This is a generated file! Please edit source .ksy file and use kaitai-struct-compiler to rebuild

package io.kaitai.struct.testformats;

import io.kaitai.struct.ByteBufferKaitaiStream;
import io.kaitai.struct.KaitaiStruct;
import io.kaitai.struct.KaitaiStream;
import java.io.IOException;

public class MetaTags extends KaitaiStruct {
    public static MetaTags fromFile(String fileName) throws IOException {
        return new MetaTags(new ByteBufferKaitaiStream(fileName));
    }

    public MetaTags(KaitaiStream _io) {
        this(_io, null, null);
    }

    public MetaTags(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public MetaTags(KaitaiStream _io, KaitaiStruct _parent, MetaTags _root) {
        super(_io);
        this._parent = _parent;
        this._root = _root == null ? this : _root;
        _read();
    }
    private void _read() {
    }
    private MetaTags _root;
    private KaitaiStruct _parent;
    public MetaTags _root() { return _root; }
    public KaitaiStruct _parent() { return _parent; }
}
