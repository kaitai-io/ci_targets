...
function SwitchManualIntSize:_read()
  ...
  while not self._io:is_eof() do
    self.chunks[i] = SwitchManualIntSize.Chunk(self._io, self, self._root)
    i = i + 1
  end
end
...
function SwitchManualIntSize.Chunk:_read()
  ...
  if _on == 17 then
    ...
    self.body = SwitchManualIntSize.Chunk.ChunkMeta(io, self, self._root)
  elseif _on == 34 then
    self._raw_body = self._io:read_bytes(self.size)
    local io = KaitaiStream(stringstream(self._raw_body))
    self.body = SwitchManualIntSize.Chunk.ChunkDir(io, self, self._root)
  else
