...
void debugEnumName_t::_read() {
    ...
    m_array_of_ints = new std::vector<testEnum2_t>();
    m_array_of_ints->reserve(l_array_of_ints);
    for (int i = 0; i < l_array_of_ints; i++) {
        m_array_of_ints->push_back(static_cast<debugEnumName_t::testEnum2_t>(m__io->read_u1()));
    }
    m_test_type = new testSubtype_t(m__io, this, m__root);
    m_test_type->_read();
}
