...
#include "valid_fail_repeat_max_int.h"
...
void valid_fail_repeat_max_int_t::_read() {
    m_foo = new std::vector<uint8_t>();
    {
        int i = 0;
        while (!m__io->is_eof()) {
            m_foo->push_back(m__io->read_u1());
            if (!(m_foo->at(i) <= 254)) {
                throw kaitai::validation_greater_than_error<uint8_t>(254, m_foo->at(i), m__io, std::string("/seq/0"));
            }
            i++;
        }
    }
