...
impl KaitaiStruct for ProcessRepeatUsertype {
    ...
    fn read<S: KaitaiStream>(&mut self,
        ...
        self.blocks = vec!();
        for i in 0..2 {
            self._raw__raw_blocks.append(self.stream.read_bytes(5)?);
            self._raw_blocks.append(&mut S::processXorOne(self._raw__raw_blocks.last(), 158));
            let mut io = Cursor::new(self._raw_blocks.last());
            self.blocks.append(Box::new(ProcessRepeatUsertype__Block::new(self.stream, self, _root)?));
        }
    }
}
