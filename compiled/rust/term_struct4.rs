...
pub struct TermStruct4 {
    pub _root: SharedType<TermStruct4>,
    pub _parent: SharedType<TermStruct4>,
...
impl KStruct for TermStruct4 {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.s1_raw.borrow_mut() = _io.read_bytes(3 as usize)?.into();
        ...
        let t = Self::read_into::<BytesReader, TermStruct4_S1Type>(&_t_s1_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
        ...
        *self_rc.s2_raw.borrow_mut() = _io.read_bytes(3 as usize)?.into();
        ...
        let t = Self::read_into::<BytesReader, TermStruct4_S2Type>(&_t_s2_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
        ...
        *self_rc.s3_raw.borrow_mut() = _io.read_bytes(3 as usize)?.into();
        ...
        let t = Self::read_into::<BytesReader, TermStruct4_S3Type>(&_t_s3_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
...
impl KStruct for TermStruct4_S1Type {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = _io.clone();
        ...
        let _prc = self_rc._parent.get_value().borrow().upgrade();
        let _r = _rrc.as_ref().unwrap();
        *self_rc.value_raw.borrow_mut() = _io.read_bytes_term(124, false, true, false)?.into();
        let value_raw = self_rc.value_raw.borrow();
        let _t_value_raw_io = BytesReader::from(value_raw.clone());
        let t = Self::read_into::<BytesReader, TermStruct4_BytesWrapper>(&_t_value_raw_io, Some(self_rc._root.clone()), None)?.into();
...
impl KStruct for TermStruct4_S2Type {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = _io.clone();
        ...
        let _prc = self_rc._parent.get_value().borrow().upgrade();
        let _r = _rrc.as_ref().unwrap();
        *self_rc.value_raw.borrow_mut() = _io.read_bytes_term(124, false, false, false)?.into();
        let value_raw = self_rc.value_raw.borrow();
        let _t_value_raw_io = BytesReader::from(value_raw.clone());
        let t = Self::read_into::<BytesReader, TermStruct4_BytesWrapper>(&_t_value_raw_io, Some(self_rc._root.clone()), None)?.into();
        *self_rc.value.borrow_mut() = t;
...
impl KStruct for TermStruct4_S3Type {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = _io.clone();
        ...
        let _prc = self_rc._parent.get_value().borrow().upgrade();
        let _r = _rrc.as_ref().unwrap();
        *self_rc.value_raw.borrow_mut() = _io.read_bytes_term(64, true, true, false)?.into();
        let value_raw = self_rc.value_raw.borrow();
        let _t_value_raw_io = BytesReader::from(value_raw.clone());
        let t = Self::read_into::<BytesReader, TermStruct4_BytesWrapper>(&_t_value_raw_io, Some(self_rc._root.clone()), None)?.into();
        *self_rc.value.borrow_mut() = t;
