...
pub struct RepeatUntilBytesPadTerm {
    pub _root: SharedType<RepeatUntilBytesPadTerm>,
    pub _parent: SharedType<RepeatUntilBytesPadTerm>,
        ...
        *self_rc.records.borrow_mut() = Vec::new();
        {
            let mut _i = 0;
            while {
                self_rc.records.borrow_mut().push(bytes_terminate(&bytes_strip_right(&_io.read_bytes(5 as usize)?.into(), 170).into(), 85, true).into());
                let _t_records = self_rc.records.borrow();
                let _tmpa = _t_records.last().unwrap();
                _i += 1;
                let x = !(_tmpa == vec![0xaau8, 0x55u8]);
                x
            } {}
        }
        Ok(())
