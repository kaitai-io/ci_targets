...
pub struct TermStruct {
    pub _root: SharedType<TermStruct>,
    pub _parent: SharedType<TermStruct>,
...
impl KStruct for TermStruct {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.s1_raw.borrow_mut() = _io.read_bytes_term(124, false, true, true)?.into();
        ...
        let t = Self::read_into::<BytesReader, TermStruct_BytesWrapper>(&_t_s1_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
        *self_rc.s1.borrow_mut() = t;
        *self_rc.s2_raw.borrow_mut() = _io.read_bytes_term(124, false, false, true)?.into();
        let s2_raw = self_rc.s2_raw.borrow();
        let _t_s2_raw_io = BytesReader::from(s2_raw.clone());
        let t = Self::read_into::<BytesReader, TermStruct_BytesWrapper>(&_t_s2_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
        *self_rc.s2.borrow_mut() = t;
        *self_rc.s3_raw.borrow_mut() = _io.read_bytes_term(64, true, true, true)?.into();
        let s3_raw = self_rc.s3_raw.borrow();
        let _t_s3_raw_io = BytesReader::from(s3_raw.clone());
        let t = Self::read_into::<BytesReader, TermStruct_BytesWrapper>(&_t_s3_raw_io, Some(self_rc._root.clone()), Some(self_rc._self.clone()))?.into();
