...
########################################################################
package ProcessCoerceUsertype1;
...
sub _read {
    ...
    for (my $i = 0; $i < $n_records; $i++) {
        push @{$self->{records}}, ProcessCoerceUsertype1::Record->new($self->{_io}, $self, $self->{_root});
    }
}
...
########################################################################
package ProcessCoerceUsertype1::Record;
...
sub _read {
    ...
    if ($self->flag() == 0) {
        ...
        $self->{buf_unproc} = ProcessCoerceUsertype1::Foo->new($io__raw_buf_unproc, $self, $self->{_root});
    }
    if ($self->flag() != 0) {
        ...
        $self->{buf_proc} = ProcessCoerceUsertype1::Foo->new($io__raw_buf_proc, $self, $self->{_root});
    }
}
...
########################################################################
package ProcessCoerceUsertype1::Foo;
