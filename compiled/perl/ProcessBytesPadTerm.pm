...
sub _read {
    ...
    $self->{str_pad} = IO::KaitaiStruct::Stream::process_xor_one($self->{_raw_str_pad}, 21);
    $self->{_raw_str_term} = IO::KaitaiStruct::Stream::bytes_terminate($self->{_io}->read_bytes(20), 64, 0);
    $self->{str_term} = IO::KaitaiStruct::Stream::process_xor_one($self->{_raw_str_term}, 21);
    ...
    $self->{str_term_and_pad} = IO::KaitaiStruct::Stream::process_xor_one($self->{_raw_str_term_and_pad}, 21);
    $self->{_raw_str_term_include} = IO::KaitaiStruct::Stream::bytes_terminate($self->{_io}->read_bytes(20), 64, 1);
    $self->{str_term_include} = IO::KaitaiStruct::Stream::process_xor_one($self->{_raw_str_term_include}, 21);
}
