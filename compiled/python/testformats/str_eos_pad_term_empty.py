...
class StrEosPadTermEmpty(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.str_pad = StrEosPadTermEmpty.StrPadType(_io_str_pad, self, self._root)
        _io_str_term = self._io.substream(20)
        self.str_term = StrEosPadTermEmpty.StrTermType(_io_str_term, self, self._root)
        _io_str_term_and_pad = self._io.substream(20)
        self.str_term_and_pad = StrEosPadTermEmpty.StrTermAndPadType(_io_str_term_and_pad, self, self._root)
        _io_str_term_include = self._io.substream(20)
        self.str_term_include = StrEosPadTermEmpty.StrTermIncludeType(_io_str_term_include, self, self._root)
