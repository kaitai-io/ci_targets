...
class NavRoot(ReadWriteKaitaiStruct):
    ...
    class Entry(ReadWriteKaitaiStruct):
        ...
        def _check(self):
            if len((self.filename).encode("UTF-8")) != self._root.header.filename_len:
                raise kaitaistruct.ConsistencyError("filename", self._root.header.filename_len, len((self.filename).encode("UTF-8")))
            self._dirty = False
    ...
    class IndexObj(ReadWriteKaitaiStruct):
        ...
        def _check(self):
            ...
            if len(self.entries) != self._root.header.qty_entries:
                raise kaitaistruct.ConsistencyError("entries", self._root.header.qty_entries, len(self.entries))
            for i in range(len(self.entries)):
                pass
