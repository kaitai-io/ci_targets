...
class ProcessBytesPadTerm(ReadWriteKaitaiStruct):
    ...
    def _write__seq(self, io=None):
        ...
        if len(self._raw_str_pad) > 20:
            raise kaitaistruct.ConsistencyError("str_pad", 20, len(self._raw_str_pad))
        if  ((len(self._raw_str_pad) != 0) and (self._raw_str_pad[-1] == 64)) :
            raise kaitaistruct.ConsistencyError("str_pad", 64, self._raw_str_pad[-1])
        ...
        if len(self._raw_str_term) > 20:
            raise kaitaistruct.ConsistencyError("str_term", 20, len(self._raw_str_term))
        if KaitaiStream.byte_array_index_of(self._raw_str_term, 64) != -1:
            raise kaitaistruct.ConsistencyError("str_term", -1, KaitaiStream.byte_array_index_of(self._raw_str_term, 64))
        ...
        if len(self._raw_str_term_and_pad) > 20:
            raise kaitaistruct.ConsistencyError("str_term_and_pad", 20, len(self._raw_str_term_and_pad))
        if KaitaiStream.byte_array_index_of(self._raw_str_term_and_pad, 64) != -1:
            raise kaitaistruct.ConsistencyError("str_term_and_pad", -1, KaitaiStream.byte_array_index_of(self._raw_str_term_and_pad, 64))
        if len(self._raw_str_term_and_pad) == 20:
            pass
            if  ((len(self._raw_str_term_and_pad) != 0) and (self._raw_str_term_and_pad[-1] == 43)) :
                raise kaitaistruct.ConsistencyError("str_term_and_pad", 43, self._raw_str_term_and_pad[-1])
        ...
        if len(self._raw_str_term_include) > 20:
            raise kaitaistruct.ConsistencyError("str_term_include", 20, len(self._raw_str_term_include))
        if len(self._raw_str_term_include) < 20:
            pass
            if len(self._raw_str_term_include) == 0:
                raise kaitaistruct.ConsistencyError("str_term_include", 0, len(self._raw_str_term_include))
            if KaitaiStream.byte_array_index_of(self._raw_str_term_include, 64) != len(self._raw_str_term_include) - 1:
                raise kaitaistruct.ConsistencyError("str_term_include", len(self._raw_str_term_include) - 1, KaitaiStream.byte_array_index_of(self._raw_str_term_include, 64))
