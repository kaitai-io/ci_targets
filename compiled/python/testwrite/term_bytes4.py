    ...
    class S1Type(ReadWriteKaitaiStruct):
        ...
        def _check(self):
            if KaitaiStream.byte_array_index_of(self.value, 124) != -1:
                raise kaitaistruct.ConsistencyError("value", -1, KaitaiStream.byte_array_index_of(self.value, 124))
            self._dirty = False
    ...
    class S3Type(ReadWriteKaitaiStruct):
        ...
        def _check(self):
            if  ((KaitaiStream.byte_array_index_of(self.value, 64) != -1) and (KaitaiStream.byte_array_index_of(self.value, 64) != len(self.value) - 1)) :
                raise kaitaistruct.ConsistencyError("value", len(self.value) - 1, KaitaiStream.byte_array_index_of(self.value, 64))
            self._dirty = False
