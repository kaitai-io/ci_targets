...
class BytesPadTermRoundtrip(ReadWriteKaitaiStruct):
    ...
    def _check(self):
        ...
        if len(self.str_term) == 20:
            pass
            if  ((len(self.str_term) != 0) and (self.str_term[-1] == 43)) :
                raise kaitaistruct.ConsistencyError("str_term", 43, self.str_term[-1])
        ...
        if KaitaiStream.byte_array_index_of(self.str_term_include, 64) == -1:
            pass
            if  ((len(self.str_term_include) != 0) and (self.str_term_include[-1] == 46)) :
                raise kaitaistruct.ConsistencyError("str_term_include", 46, self.str_term_include[-1])
