...
class StrPadTermRoundtrip(ReadWriteKaitaiStruct):
    def __init__(self, _io=None, _parent=None, _root=None):
        super(StrPadTermRoundtrip, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self

    def _read(self):
        self.str_pad = (KaitaiStream.bytes_strip_right(self._io.read_bytes(20), 64)).decode(u"UTF-8")
        self.str_term = (KaitaiStream.bytes_terminate(KaitaiStream.bytes_strip_right(self._io.read_bytes(20), 43), 64, False)).decode(u"UTF-8")
        ...
        self.str_term_include = (KaitaiStream.bytes_terminate(KaitaiStream.bytes_strip_right(self._io.read_bytes(20), 46), 64, True)).decode(u"UTF-8")
        self._dirty = False
    ...
    def _write__seq(self, io=None):
        super(StrPadTermRoundtrip, self)._write__seq(io)
        self._io.write_bytes_limit((self.str_pad).encode(u"UTF-8"), 20, 64, 64)
        self._io.write_bytes_limit((self.str_term).encode(u"UTF-8"), 20, 64, 43)
        self._io.write_bytes_limit((self.str_term_and_pad).encode(u"UTF-8"), 20, 64, 43)
        self._io.write_bytes_limit((self.str_term_include).encode(u"UTF-8"), 20, 46, 46)
