...
class StrEncodingsEscapingEnc(ReadWriteKaitaiStruct):
    ...
    def _write__seq(self, io=None):
        ...
        _io__raw_str1 = KaitaiStream(BytesIO(bytes(self.len_of_1)))
        ...
        def handler(parent, _io__raw_str1=_io__raw_str1):
            ...
            if len(self._raw_str1) != self.len_of_1:
                raise kaitaistruct.ConsistencyError("raw(str1)", self.len_of_1, len(self._raw_str1))
        ...
        _io__raw_str2 = KaitaiStream(BytesIO(bytes(self.len_of_2)))
        ...
        def handler(parent, _io__raw_str2=_io__raw_str2):
            ...
            if len(self._raw_str2) != self.len_of_2:
                raise kaitaistruct.ConsistencyError("raw(str2)", self.len_of_2, len(self._raw_str2))
        ...
        _io__raw_str3 = KaitaiStream(BytesIO(bytes(self.len_of_3)))
        ...
        def handler(parent, _io__raw_str3=_io__raw_str3):
            ...
            if len(self._raw_str3) != self.len_of_3:
                raise kaitaistruct.ConsistencyError("raw(str3)", self.len_of_3, len(self._raw_str3))
        ...
        _io__raw_str4 = KaitaiStream(BytesIO(bytes(self.len_of_4)))
        ...
        def handler(parent, _io__raw_str4=_io__raw_str4):
            ...
            if len(self._raw_str4) != self.len_of_4:
                raise kaitaistruct.ConsistencyError("raw(str4)", self.len_of_4, len(self._raw_str4))
    ...
    def _check(self):
        if self.str1._root != self._root:
            raise kaitaistruct.ConsistencyError("str1", self._root, self.str1._root)
        if self.str1._parent != self:
            raise kaitaistruct.ConsistencyError("str1", self, self.str1._parent)
        if self.str2._root != self._root:
            raise kaitaistruct.ConsistencyError("str2", self._root, self.str2._root)
        if self.str2._parent != self:
            raise kaitaistruct.ConsistencyError("str2", self, self.str2._parent)
        if self.str3._root != self._root:
            raise kaitaistruct.ConsistencyError("str3", self._root, self.str3._root)
        if self.str3._parent != self:
            raise kaitaistruct.ConsistencyError("str3", self, self.str3._parent)
        if self.str4._root != self._root:
            raise kaitaistruct.ConsistencyError("str4", self._root, self.str4._root)
        if self.str4._parent != self:
            raise kaitaistruct.ConsistencyError("str4", self, self.str4._parent)
        self._dirty = False
    ...
    class Str1Wrapper(ReadWriteKaitaiStruct):
        ...
        def _write_v(self):
            ...
            _pos = self._io.pos()
            self._io.seek(0)
            self._io.write_bytes((self._m_v).encode("ASCII\\\\x"))
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("v", 0, self._io.size() - self._io.pos())
            self._io.seek(_pos)
    ...
    class Str2Wrapper(ReadWriteKaitaiStruct):
        ...
        def _write_v(self):
            ...
            _pos = self._io.pos()
            self._io.seek(0)
            self._io.write_bytes((self._m_v).encode("UTF-8\\'x"))
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("v", 0, self._io.size() - self._io.pos())
            self._io.seek(_pos)
    ...
    class Str3Wrapper(ReadWriteKaitaiStruct):
        ...
        def _write_v(self):
            ...
            _pos = self._io.pos()
            self._io.seek(0)
            self._io.write_bytes((self._m_v).encode("SJIS\\\"x"))
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("v", 0, self._io.size() - self._io.pos())
            self._io.seek(_pos)
    ...
    class Str4Wrapper(ReadWriteKaitaiStruct):
        ...
        def _write_v(self):
            ...
            _pos = self._io.pos()
            self._io.seek(0)
            self._io.write_bytes((self._m_v).encode("IBM437\\nx"))
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("v", 0, self._io.size() - self._io.pos())
            self._io.seek(_pos)
