...
class TermBytes2(ReadWriteKaitaiStruct):
    ...
    def _check(self):
        ...
        if len(self.s2) == 0:
            raise kaitaistruct.ConsistencyError(u"s2", len(self.s2), 0)
        if KaitaiStream.byte_array_index_of(self.s2, 124) != len(self.s2) - 1:
            raise kaitaistruct.ConsistencyError(u"s2", KaitaiStream.byte_array_index_of(self.s2, 124), len(self.s2) - 1)
