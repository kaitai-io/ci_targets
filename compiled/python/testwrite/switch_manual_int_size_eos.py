...
class SwitchManualIntSizeEos(ReadWriteKaitaiStruct):
    ...
    def _write__seq(self, io=None):
        super(SwitchManualIntSizeEos, self)._write__seq(io)
        for i in range(len(self.chunks)):
            pass
            if self._io.is_eof():
    ...
    class Chunk(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(SwitchManualIntSizeEos.Chunk, self)._write__seq(io)
    ...
    class ChunkBody(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(SwitchManualIntSizeEos.ChunkBody, self)._write__seq(io)
            _on = self._parent.code
            if _on == 17:
                pass
                ...
                def handler(parent, _io__raw_body=_io__raw_body):
                    ...
                    if not parent.is_eof():
                        raise kaitaistruct.ConsistencyError(u"raw(body)", parent.size() - parent.pos(), 0)
                _io__raw_body.write_back_handler = KaitaiStream.WriteBackHandler(_pos2, handler)
                self.body._write__seq(_io__raw_body)
            elif _on == 34:
                pass
                ...
                if not self._io.is_eof():
                    raise kaitaistruct.ConsistencyError(u"body", self._io.size() - self._io.pos(), 0)
        ...
        class ChunkDir(ReadWriteKaitaiStruct):
            ...
            def _write__seq(self, io=None):
                super(SwitchManualIntSizeEos.ChunkBody.ChunkDir, self)._write__seq(io)
                for i in range(len(self.entries)):
                    pass
                    if self._io.is_eof():
        ...
        class ChunkMeta(ReadWriteKaitaiStruct):
            ...
            def _write__seq(self, io=None):
                super(SwitchManualIntSizeEos.ChunkBody.ChunkMeta, self)._write__seq(io)
