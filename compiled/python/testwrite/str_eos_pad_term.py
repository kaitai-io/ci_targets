...
class StrEosPadTerm(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        self.str_pad = StrEosPadTerm.StrPadType(_io__raw_str_pad, self, self._root)
        self.str_pad._read()
        self._raw_str_term = self._io.read_bytes(20)
        _io__raw_str_term = KaitaiStream(BytesIO(self._raw_str_term))
        self.str_term = StrEosPadTerm.StrTermType(_io__raw_str_term, self, self._root)
        self.str_term._read()
        ...
        self.str_term_and_pad = StrEosPadTerm.StrTermAndPadType(_io__raw_str_term_and_pad, self, self._root)
        ...
        self.str_term_include = StrEosPadTerm.StrTermIncludeType(_io__raw_str_term_include, self, self._root)
    ...
    def _write__seq(self, io=None):
        ...
        _io__raw_str_pad = KaitaiStream(BytesIO(bytes(20)))
        ...
        _io__raw_str_term = KaitaiStream(BytesIO(bytes(20)))
        ...
        _io__raw_str_term_and_pad = KaitaiStream(BytesIO(bytes(20)))
        ...
        _io__raw_str_term_include = KaitaiStream(BytesIO(bytes(20)))
    ...
    class StrTermAndPadType(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super()._write__seq(io)
            self._io.write_bytes_limit((self.value).encode("UTF-8"), self._io.size() - self._io.pos(), 64, 43)
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("value", 0, self._io.size() - self._io.pos())
    ...
    class StrTermIncludeType(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super()._write__seq(io)
            self._io.write_bytes_limit((self.value).encode("UTF-8"), self._io.size() - self._io.pos(), 0, 0)
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("value", 0, self._io.size() - self._io.pos())
    ...
    class StrTermType(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super()._write__seq(io)
            self._io.write_bytes_limit((self.value).encode("UTF-8"), self._io.size() - self._io.pos(), 64, 0)
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError("value", 0, self._io.size() - self._io.pos())
