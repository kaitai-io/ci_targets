...
class NavParentVsValueInst(ReadWriteKaitaiStruct):
    ...
    def _check(self):
        ...
        if self.child._root != self._root:
            raise kaitaistruct.ConsistencyError("child", self._root, self.child._root)
        if self.child._parent != self:
            raise kaitaistruct.ConsistencyError("child", self, self.child._parent)
        self._dirty = False
    ...
    class ChildObj(ReadWriteKaitaiStruct):
        ...
        def do_something(self):
            ...
            self._m_do_something = (True if self._parent.s1 == "foo" else False)
