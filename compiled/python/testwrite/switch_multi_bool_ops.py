...
class SwitchMultiBoolOps(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        while not self._io.is_eof():
            _t_opcodes = SwitchMultiBoolOps.Opcode(self._io, self, self._root)
            try:
                _t_opcodes._read()
            finally:
    ...
    def _write__seq(self, io=None):
        super(SwitchMultiBoolOps, self)._write__seq(io)
        for i in range(len(self.opcodes)):
            pass
            if self._io.is_eof():
    ...
    class Opcode(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(SwitchMultiBoolOps.Opcode, self)._write__seq(io)
