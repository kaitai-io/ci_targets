...
pub const ValidFailRepeatContents = struct {
    pub fn create(_arena: *std.heap.ArenaAllocator, _io: *kaitai_struct.KaitaiStream, _parent: ?*anyopaque, _root: ?*ValidFailRepeatContents) !*ValidFailRepeatContents {
    ...
    fn _allocator(self: *const ValidFailRepeatContents) std.mem.Allocator {
    ...
    fn _read(self: *ValidFailRepeatContents) !void {
        ...
        self.foo.* = .empty;
        {
            ...
            while (!try self._io.isEof()) : (i += 1) {
                ...
                if (!(std.mem.eql(u8, self.foo.items[i], &[_]u8{ 18, 52, 86, 120 }))) {
