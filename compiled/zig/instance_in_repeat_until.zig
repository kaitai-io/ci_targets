...
pub const InstanceInRepeatUntil = struct {
    pub fn create(_arena: *std.heap.ArenaAllocator, _io: *kaitai_struct.KaitaiStream, _parent: ?*anyopaque, _root: ?*InstanceInRepeatUntil) !*InstanceInRepeatUntil {
    ...
    fn _allocator(self: *const InstanceInRepeatUntil) std.mem.Allocator {
        return self._arena.allocator();
    }
    fn _read(self: *InstanceInRepeatUntil) !void {
        self.entries = try self._allocator().create(std.ArrayList(i16));
        self.entries.* = .empty;
        {
            var i: usize = 0;
            while (true) : (i += 1) {
    ...
    entries: *std.ArrayList(i16) = undefined,
